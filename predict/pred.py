...
from Utiles.utile import pad, convert_to_square, calibrate_box, processed_image
from Utiles.GenerateBox import generate_bbox

parser = argparse.ArgumentParser()
parser.add_argument('--model_path', type=str, default='../models', help='PNet、RNet、ONet三个模型文件存在的文件夹路径')
parser.add_argument('--image_path', type=str, default='D:\\test.jpg', help='需要预测图像的路径')
...
def predict_pnet(infer_data):
    ...
    infer_data = torch.unsqueeze(infer_data, dim=0)  # 将输入增加一个维度，dim决定在哪一层增加一个维度
    # 执行预测
    cls_prob, bbox_pred, _ = pnet(infer_data)
    cls_prob = torch.squeeze(cls_prob)  # 降维
    cls_prob = softmax_p(cls_prob)  # 激活函数
    bbox_pred = torch.squeeze(bbox_pred)  # 降维
...
def detect_pnet(im, min_face_size, scale_factor, thresh):
    ...
    while min(current_height, current_width) > net_size:
        # 类别和box
        # print("resize后的金字塔图像size:", im_resized.shape)
        # pnet模型预测
        ...
        # 非极大值抑制留下重复低的box，并使用all_boxes进行保存
...
def detect_rnet(im, dets, thresh):
    ...
    """
    h, w, c = im.shape
    # 将pnet的box变成包含它的正方形，可以避免信息损失,使box宽变大
    dets = convert_to_square(dets)
    dets[:, 0:4] = np.round(dets[:, 0:4])  # 边长取整
    # 调整超出图像的box
...
def draw_face(image_path, boxes_c, landmarks):
    ...
    for i in range(landmarks.shape[0]):
        for j in range(len(landmarks[i]) // 2):
            cv2.circle(img, (int(landmarks[i][2 * j]), int(int(landmarks[i][2 * j + 1]))), 2, (0, 255, 0))
    cv2.imshow('result', img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()


def hook_pic(img, locations):
    """
    img:原始图片
    locations: 模型识别出来的人脸框
    """
    im_cp = img.copy()
    for i in range(locations.shape[0]):
        locat = locations[i, :]
        cv2.rectangle(im_cp, (int(locat[0]), int(locat[1])), (int(locat[2]), int(locat[3])), (0, 255, 0), 1)
        cv2.imshow("test", im_cp)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
...
if __name__ == '__main__':
    ...
    endtime = time.time()
    print("模型执行时间：{} ms".format((endtime-startime)*1000))
    # 把关键画出来
