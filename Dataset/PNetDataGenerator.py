...
@File    ：PNetDataGenerator.py
@Author  ：huangxj
@Date    ：2023/9/7 11:07
PNet网络训练数据生成器
从人脸库中生成可使用的数据
1、从/WIDER_train/images 以及 wider_face_train.txt 中获取人脸图片及数据标注
'''
...
def crop_12_box_image(data_path):
    ...
    # 记录读取图片数
    idx = 0
    for annotation in tqdm(annotations[:50]):  # 测试50张
        ...
        while neg_num < 50:
            ...
            # 截取box,随机截取大小为size的图片
            crop_box = np.array([nx, ny, nx + size, ny + size])
            # 计算iou值。保存截取图片框和标记的人脸框重合率小于阈值
...
if __name__ == '__main__':
    data_path = confs["data-base"]
    face_boxs = confs["face-box-train-label"]
    face_landmarks = confs["face-landmarks-train-label"]
    ...
    # 获取lfw negbox，关键点
    lfw_neg_path = os.path.join(data_path, face_landmarks)
